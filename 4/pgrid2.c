...
int main(int argc, char *argv[]) {
    ...
    MPI_Dims_create(commsize, 2, dims); // позволяет заданное число процессов разложить на два сомножителя
    int px = dims[0];
    int py = dims[1];
    if (px < 2 || py < 2) {
        fprintf(stderr,
    ...
    MPI_Cart_create(MPI_COMM_WORLD, 2, dims, periodic, 0, &cartcomm); // создает двумерную сетку процессов . 
    //Появляется новый коммуникатор cartcomm который будет содержать две координаты (x,y).
    int coords[2];
    MPI_Cart_coords(cartcomm, rank, 2, coords); // Каждый процесс определет свои координаты rankx ranky с помошью MPI_Cart_coords.
    int rankx = coords[0];
    int ranky = coords[1];
    int rows, cols;
    ...
    // Neighbours | с помощью MPI_Cart_shift определяем номера соседей для процесса
    ...
    for (;;) {
        ...
        // px)) (MPI_proc_NULL если соседа нет. операция просто завершается )
        thalo -= MPI_Wtime();
